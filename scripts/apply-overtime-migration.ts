/**
 * One-shot: apply docs/migrations/add-overtime-requests.sql to the database in
 * DATABASE_URL. Idempotent (CREATE TABLE/INDEX IF NOT EXISTS). Run:
 *   npx tsx --env-file=.env scripts/apply-overtime-migration.ts
 */
...
const ddl = readFileSync('docs/migrations/add-overtime-requests.sql', 'utf8');
...
async function main() {
  ...
  const [{ exists }] = await sql`select to_regclass('public.overtime_requests') is not null as exists`;
  const cols = await sql`
    select column_name from information_schema.columns
    where table_schema = 'public' and table_name = 'overtime_requests'
    order by ordinal_position`;
  console.log('overtime_requests exists:', exists);
