/**
 * One-shot: apply docs/migrations/add-presence-pings.sql to the database in
 * DATABASE_URL. Idempotent (CREATE TABLE/INDEX IF NOT EXISTS). Run:
 *   npx tsx --env-file=.env scripts/apply-presence-pings-migration.ts
 */
...
const ddl = readFileSync('docs/migrations/add-presence-pings.sql', 'utf8');
...
async function main() {
  ...
  const [{ exists }] = await sql`select to_regclass('public.presence_pings') is not null as exists`;
  const cols = await sql`
    select column_name from information_schema.columns
    where table_schema = 'public' and table_name = 'presence_pings'
    order by ordinal_position`;
  console.log('presence_pings exists:', exists);
