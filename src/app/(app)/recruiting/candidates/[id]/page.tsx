...
} from "../pre-employment-actions";
import {
  EmploymentVerificationResponseModal,
  ReferenceResponseModal,
} from "../reference-response-modal";
import {
  MarkCandidateSowSignedButton,
  PreEmploymentDocumentUpload,
...
type CandidatePreEmploymentDocumentRow = {
  kind: "sow" | "job_description" | "ai_policy" | "nda";
  file_name: string;
  signed_url: string;
  uploaded_at: string;
  sent_at: string | null;
...
function backgroundCheckEntries(payload: unknown): {
  ...
  const references = [1, 2, 3]
    ...
    .filter(
      (reference) => reference.name || reference.email || reference.phone,
    );
  const verifications = [1, 2, 3]
    .map((index) => ({
...
export default async function CandidateDetailPage({
  ...
  if (
    !session ||
    !hasToolAccess("recruiting", session.role, session.toolAccess)
  ) {
  ...
  const activePreEmploymentTab: PreEmploymentTab =
    VALID_PRE_EMPLOYMENT_TABS.includes(
  ...
  if (showPreEmployment) {
    ...
    if (!submissionResult.error && submissionResult.data) {
      backgroundCheckSubmission =
        submissionResult.data as PreEmploymentSubmissionRow;
    }
    ...
    if (candidateReferences.length > 0) {
      const {
        data: referenceSubmissionData,
        error: referenceSubmissionsError,
      } = await supabase
        .from("candidate_reference_form_submissions")
        .select("reference_id, submitted_at, payload")
        .in(
          "reference_id",
          candidateReferences.map((reference) => reference.id),
        )
        .order("submitted_at", { ascending: false });
      if (!referenceSubmissionsError && referenceSubmissionData) {
        candidateReferenceSubmissions =
          referenceSubmissionData as CandidateReferenceSubmissionRow[];
      }
    }
    ...
    if (!employmentError && employmentData) {
      candidateEmploymentVerifications =
        employmentData as CandidateEmploymentVerificationRow[];
    }
    if (candidateEmploymentVerifications.length > 0) {
      const {
        data: employmentSubmissionData,
        error: employmentSubmissionsError,
      } = await supabase
        .from("candidate_employment_verification_form_submissions")
        .select("verification_id, submitted_at, payload")
        .in(
          "verification_id",
          candidateEmploymentVerifications.map(
            (verification) => verification.id,
          ),
        )
        .order("submitted_at", { ascending: false });
      if (!employmentSubmissionsError && employmentSubmissionData) {
        candidateEmploymentVerificationSubmissions =
          employmentSubmissionData as CandidateEmploymentVerificationSubmissionRow[];
      }
    }
    ...
    if (!documentsError && documentsData) {
      ...
      const { data: legacyDocuments, error: legacyDocumentsError } =
        await supabase
          .from("candidate_pre_employment_documents")
          .select("kind, file_name, signed_url, uploaded_at, sent_at")
          .eq("candidate_id", id);
      if (!legacyDocumentsError && legacyDocuments) {
        candidateDocuments = (
          legacyDocuments as Array<
            Omit<CandidatePreEmploymentDocumentRow, "signed_at">
          >
  ...
  const lastEmailEvent =
    history.find(
      (h) => h.field === "email_sent" || h.field === "email_failed",
    ) ?? null;
  const lastFailedEmailContext =
    lastEmailEvent && lastEmailEvent.field === "email_failed"
          ...
          <div className="flex flex-col md:flex-row md:items-start gap-5">
            ...
            <div className="min-w-0 flex-1 space-y-3">
                ...
                <div className="min-w-0">
                  ...
                  {c.position && (
                    <p className="mt-0.5 text-sm text-(--rs-neutral-grey-600)">
                      Applying for{" "}
                      <strong className="text-(--rs-neutral-grey-900)">
                        {c.position}
                      </strong>
                    </p>
                  )}
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm text-(--rs-neutral-grey-700)">
                ...
                {c.phone && (
                  <ContactPill
                    ...
                    text={formatPhoneNumber(c.phone)}
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1 text-xs text-(--rs-neutral-grey-500)">
                ...
                {addedByName ? (
                  <span className="inline-flex items-center gap-1.5">
                    <User2 className="w-3 h-3" /> Added by{" "}
                    <strong className="text-(--rs-neutral-grey-700)">
                      {addedByName}
                    </strong>
                  </span>
                ) : selfApplied ? (
                ...
                {c.parsed_at && (
                  <span className="inline-flex items-center gap-1.5 text-(--rs-primary-600)">
                    <Sparkles className="w-3 h-3" /> Resume parsed{" "}
                    {formatDate(c.parsed_at)}
                  </span>
                )}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <Section title={`Skills · ${c.skills!.length}`}>
              <div className="flex flex-wrap gap-1.5">
                {c.skills!.map((s, i) => (
                  <span
                    key={i}
                    className="rounded-md bg-(--rs-primary-50) px-2.5 py-1 text-xs font-medium text-(--rs-primary-800)"
                  >
                    {s}
                  </span>
                ))}
          ...
          {!showPreEmployment && (c.experience?.length ?? 0) > 0 && (
            <Section
              title={`Experience · ${c.experience!.length}`}
                  ...
                  <li key={i} className="relative">
                    ...
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <h4 className="font-semibold text-(--rs-neutral-grey-900)">
                        {e.title || "Role"}{" "}
                        {e.company && (
                          <span className="font-normal text-(--rs-neutral-grey-600)">
                            @ {e.company}
                          </span>
                        )}
                      </h4>
                    ...
                    {e.description && (
                      <p className="mt-1.5 text-sm text-(--rs-neutral-grey-700) leading-relaxed whitespace-pre-wrap">
                        {e.description}
                      </p>
                    )}
                  </li>
          ...
          {!showPreEmployment && (c.education?.length ?? 0) > 0 && (
            <Section
              title={`Education · ${c.education!.length}`}
                  ...
                  >
                    <div className="flex flex-wrap gap-1.5">
                      {c.languages!.map((lang, i) => (
                        <span
                          key={i}
                          className="rounded-md bg-(--rs-neutral-grey-100) px-2.5 py-1 text-xs font-medium text-(--rs-neutral-grey-700)"
                        >
                          {lang}
                        </span>
                      ))}
          ...
          {showPreEmployment && (
            <>
              ...
              {activePreEmploymentTab === "documents" && (
                <PreEmploymentDocumentsTab
                  candidateId={c.id}
                  canUpload={c.status === "offered"}
                  documents={candidateDocuments}
                />
              )}
            </>
        ...
        <aside className="space-y-5">
              ...
              <div className="flex gap-2 text-(--rs-accent-700)">
                ...
                <div>
                  ...
                  <p className="text-xs mt-1 text-(--rs-accent-700)/80">
                    Upload a PDF/DOCX above to auto-fill summary, skills,
          ...
          <div className="rounded-xl border border-(--rs-neutral-grey-200) bg-white p-4 space-y-2.5">
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-(--rs-neutral-grey-500)">
              Actions
            </h3>
            {c.resume_url && (
              <a
              ...
              <ol className="space-y-2 max-h-[28rem] overflow-y-auto pr-1">
                {history.map((h) => (
                  <li key={h.id} className="text-xs leading-snug">
                    ...
                    <span className="font-semibold text-(--rs-neutral-grey-800)">
                      {abbreviateName(h.user_name)}
                    </span>
                    <span className="text-(--rs-neutral-grey-400)">: </span>
                    <span className="text-(--rs-neutral-grey-700)">
                      {h.summary}
                    </span>
                  </li>
                ))}
...
function PreEmploymentBackgroundCheckTab({
  ...
  const { references: payloadReferences, verifications: payloadVerifications } =
    backgroundCheckEntries(submission?.payload);
  const submissionsByReferenceId = new Map<
    number,
    CandidateReferenceSubmissionRow
  >();
  for (const referenceSubmission of candidateReferenceSubmissions) {
    if (!submissionsByReferenceId.has(referenceSubmission.reference_id)) {
      submissionsByReferenceId.set(
        referenceSubmission.reference_id,
        referenceSubmission,
      );
    }
  }
  const references: CharacterReference[] =
    candidateReferences.length > 0
      ...
      : payloadReferences;
  const employmentSubmissionsByVerificationId = new Map<
    number,
    CandidateEmploymentVerificationSubmissionRow
  >();
  for (const employmentSubmission of candidateEmploymentVerificationSubmissions) {
    if (
      !employmentSubmissionsByVerificationId.has(
        employmentSubmission.verification_id,
      )
    ) {
      employmentSubmissionsByVerificationId.set(
        employmentSubmission.verification_id,
        employmentSubmission,
      );
    }
  }
  const verifications: EmploymentVerification[] =
    candidateEmploymentVerifications.length > 0
      ? candidateEmploymentVerifications.map((verification) => ({
          id: verification.id,
          company: verification.company,
          contactName: verification.hr_contact_name ?? "",
          email: verification.hr_email,
  ...
  const allThreeReferencesReady =
    candidateReferences.length === 3 &&
    candidateReferences.every(
      (reference) =>
        !!reference.referee_name.trim() && !!reference.referee_email.trim(),
    );
  const unsentReferenceCount = candidateReferences.filter(
    (reference) => !reference.request_sent_at,
  ).length;
  const unsentEmploymentVerificationCount =
...
function PreEmploymentCandidateInformationTab({
  candidate,
}: {
  candidate: Candidate;
}) {
    ...
    <Section title="Resume Information" icon={<FileText className="w-4 h-4" />}>
      {!hasResumeInformation ? (
        ...
        <div className="space-y-5">
          {candidate.summary && (
            <p className="whitespace-pre-wrap text-sm leading-relaxed text-(--rs-neutral-grey-700)">
              {candidate.summary}
            </p>
          )}
          ...
          {(candidate.skills?.length ?? 0) > 0 && (
            <div>
              <p className="text-xs font-semibold text-(--rs-neutral-grey-700)">
                Skills
              </p>
            ...
            <div>
              ...
              <div className="mt-2 space-y-3">
                {candidate.experience!.map((experience, index) => (
                  <div
                    key={index}
                    className="rounded-lg border border-(--rs-neutral-grey-200) bg-white p-3"
                  >
                    <p className="text-sm font-semibold text-(--rs-neutral-grey-900)">
                    ...
                    {experience.description && (
                      <p className="mt-2 whitespace-pre-wrap text-xs leading-relaxed text-(--rs-neutral-grey-700)">
                        {experience.description}
                      </p>
                    )}
                  </div>
              ...
              <div className="mt-2 space-y-3">
                {candidate.education!.map((education, index) => (
                  ...
                  >
                    ...
                    <p className="mt-0.5 text-xs text-(--rs-neutral-grey-600)">
...
function PreEmploymentNextStep({
  candidateStatus,
  request,
  submission,
  references,
  ...
  employmentVerifications: CandidateEmploymentVerificationRow[];
  documents: CandidatePreEmploymentDocumentRow[];
}) {
  type Progress = {
    title: string;
    detail: string;
  ...
  const unsentReferences = references.filter((reference) => !reference.request_sent_at).length;
  const pendingReferences = references.filter(
    (reference) => !!reference.request_sent_at && !reference.responded_at,
  ).length;
  const unsentEmploymentVerifications = employmentVerifications.filter(
    (verification) => !verification.request_sent_at,
  ).length;
  const pendingEmploymentVerifications = employmentVerifications.filter(
    (verification) => !!verification.request_sent_at && !verification.responded_at,
  ).length;
  const allDocumentKinds: CandidatePreEmploymentDocumentRow["kind"][] = [
    "sow",
    "job_description",
    "ai_policy",
    "nda",
  ...
  const missingDocuments = allDocumentKinds.filter((kind) => !uploadedKinds.has(kind));
  const sow = documents.find((document) => document.kind === "sow");

  let progress: Progress;
  if (candidateStatus === "hired") {
    progress = {
      title: "Pre-employment complete",
      detail: "Candidate has moved to Onboarding. This Recruitment record remains available as history.",
      tone: "complete",
    };
  } else if (!submission) {
    if (request && !request.invalidated_at) {
      progress = {
        ...
        detail: `Background-check email sent ${formatDate(request.sent_at)}. The secure form link expires ${formatDate(request.expires_at)}.`,
        tone: "waiting",
      };
    } else {
      progress = {
        title: "Next action: Send background check",
        detail: "Send the candidate a secure Background Check form to collect their references and previous-employer contacts.",
        tone: "next",
      };
    }
  } else if (unsentReferences > 0 || unsentEmploymentVerifications > 0) {
    const actions = [
      unsentReferences > 0 ? "character-reference requests" : null,
      unsentEmploymentVerifications > 0 ? "employment-verification requests" : null,
    ].filter(Boolean).join(" and ");
    progress = {
      title: `Next action: Send ${actions}`,
      detail: `The candidate submitted the Background Check form ${formatDate(submission.submitted_at)}.`,
      tone: "next",
    };
  } else if (pendingReferences > 0 || pendingEmploymentVerifications > 0) {
    const waitingFor = [
      pendingReferences > 0 ? `${pendingReferences} character reference${pendingReferences === 1 ? "" : "s"}` : null,
      pendingEmploymentVerifications > 0 ? `${pendingEmploymentVerifications} employment verification${pendingEmploymentVerifications === 1 ? "" : "s"}` : null,
    ].filter(Boolean).join(" and ");
    progress = {
      title: "Awaiting external responses",
    ...
    progress = {
      title: "Next action: Send the document package",
      detail: "All required documents are uploaded. Send the SOW, Job Description, AI Policy, and NDA together.",
      tone: "next",
    };
  } else if (!sow?.signed_at) {
    progress = {
      title: "Awaiting SOW signature",
      detail: "Once the signed SOW is confirmed, mark it as signed to move the candidate into Onboarding.",
      tone: "waiting",
    };
  } else {
    progress = {
      title: "Pre-employment requirements complete",
      detail: "The candidate is ready to move to Onboarding.",
      tone: "complete",
    };
  }
  ...
  return (
    <div className={`rounded-xl border px-4 py-3 text-sm ${toneClass}`}>
      ...
      <p className={`mt-0.5 text-xs ${detailClass}`}>{progress.detail}</p>
    </div>
  );
}

function ReferenceSubmissionCard({
  reference,
  index,
}: {
  ...
  const details = [reference.jobTitle, reference.company]
    .filter(Boolean)
    .join(" · ");
  const contact = [
    reference.email,
    reference.phone && formatPhoneNumber(reference.phone),
  ]
    .filter(Boolean)
    .join(" · ");
  return (
    <div className="rounded-lg border border-(--rs-neutral-grey-200) bg-white p-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-(--rs-neutral-grey-900)">
            Reference {index + 1}: {reference.name || "Unnamed reference"}
          </p>
          {details && (
            <p className="mt-0.5 text-xs text-(--rs-neutral-grey-600)">
              {details}
        ...
        {reference.relationship && (
          <span className="rounded-full bg-(--rs-primary-50) px-2 py-0.5 text-[10px] font-semibold text-(--rs-primary-700)">
            {reference.relationship}
          </span>
        )}
      </div>
      {contact && (
        <p className="mt-2 text-xs text-(--rs-neutral-grey-700)">{contact}</p>
      )}
      {reference.bestTimeToCall && (
        <p className="mt-1 text-[11px] text-(--rs-neutral-grey-500)">
          Best time to call: {reference.bestTimeToCall}
        </p>
      )}
      {reference.requestSentAt && (
      ...
      {reference.responsePayload != null && (
        <ReferenceResponseModal
          refereeName={reference.name}
          payload={reference.responsePayload}
        />
      )}
    </div>
  );
...
function EmploymentVerificationCard({
  verification,
  index,
}: {
  ...
  const contact = [
    verification.email,
    verification.phone && formatPhoneNumber(verification.phone),
  ]
    .filter(Boolean)
    .join(" · ");
  return (
    <div className="rounded-lg border border-(--rs-neutral-grey-200) bg-white p-3">
      <p className="text-sm font-semibold text-(--rs-neutral-grey-900)">
        Employer {index + 1}: {verification.company || "Unnamed employer"}
      </p>
      {verification.contactName && (
        <p className="mt-0.5 text-xs text-(--rs-neutral-grey-600)">
          HR contact: {verification.contactName}
        </p>
      )}
      {contact && (
        <p className="mt-2 text-xs text-(--rs-neutral-grey-700)">{contact}</p>
      )}
      {verification.bestTimeToCall && (
        <p className="mt-1 text-[11px] text-(--rs-neutral-grey-500)">
          Best time to call: {verification.bestTimeToCall}
        </p>
      )}
      {verification.requestSentAt && (
        <p className="mt-1 text-[11px] font-medium text-green-700">
      ...
      {verification.respondedAt && (
        <p className="mt-1 text-[11px] font-medium text-green-700">
          Employment-verification form submitted{" "}
          {formatDate(verification.respondedAt)}
        </p>
      )}
      {verification.responsePayload != null && (
        <EmploymentVerificationResponseModal
          company={verification.company}
          payload={verification.responsePayload}
        />
      )}
    </div>
  );
  ...
  return (
    ...
    >
      <p className="text-xs leading-relaxed text-(--rs-neutral-grey-600)">
        Upload approved PDFs here. Documents remain internal until a signing
        workflow is chosen.
      </p>
      <div className="mt-3 divide-y divide-(--rs-neutral-grey-200) rounded-lg border border-(--rs-neutral-grey-200) bg-white px-3">
        <PreEmploymentDocument
          ...
          action={
            <div className="flex items-center gap-2">
              {canUpload &&
                byKind.get("sow")?.sent_at &&
                !byKind.get("sow")?.signed_at && (
                  <MarkCandidateSowSignedButton candidateId={candidateId} />
                )}
              <PreEmploymentDocumentUpload
                candidateId={candidateId}
                kind="sow"
        ...
        <PreEmploymentDocument
          ...
          document={byKind.get("job_description")}
          action={
            <PreEmploymentDocumentUpload
              candidateId={candidateId}
...
function PreEmploymentTabBar({
  id,
  active,
}: {
  id: number;
  active: PreEmploymentTab;
}) {
...
function PreEmploymentDocument({
  name,
  document,
  action,
}: {
  name: string;
  document?: CandidatePreEmploymentDocumentRow;
  action?: React.ReactNode;
}) {
  return (
      ...
      <div>
        ...
        {document && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="ml-2 text-[11px] font-semibold text-(--rs-primary-700) hover:underline"
          >
        ...
        {document?.sent_at && (
          <p className="mt-0.5 text-[10px] font-medium text-green-700">
            Sent {formatDate(document.sent_at)}
          </p>
        )}
        {document?.signed_at && (
          <p className="mt-0.5 text-[10px] font-medium text-green-700">
    ...
    <Card>
      <CardContent className="p-6 space-y-3">
        ...
        <p className="text-sm text-(--rs-neutral-grey-600)">
          The candidates table hasn&apos;t been created yet. Run the migrations
          under{" "}
