...
import { regenerateCurrentWeekStatusDraft } from './actions';

export function RegenerateStatusDraftButton({ variant = 'outline' }: { variant?: 'default' | 'outline' }) {
    ...
    <div className="inline-flex flex-col items-end gap-1">
      <Button
        ...
        onClick={() => {
          setError(null);
          start(async () => {
            try { await regenerateCurrentWeekStatusDraft(); }
            catch (err) { setError(err instanceof Error ? err.message : 'Failed'); }
          });
        }}
      >
        <RefreshCw className={`h-4 w-4 transition-transform duration-300 ${isPending ? 'animate-spin text-(--rs-primary-600)' : ''}`} />
