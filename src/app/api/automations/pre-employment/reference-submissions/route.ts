...
// n8n posts the completed referee Jotform here. It is authenticated with the
// same private automation secret as the candidate-form intake.
...
function stripRequestToken(data: unknown): unknown {
  ...
  const {
    ...
    q22_request_token: _jotformRequestToken,
    ...safeData
...
export const POST = route(async (req: Request) => {
  ...
  try {
    if (JSON.stringify(safeData).length > 1_000_000) {
  ...
  const { data, error } = await supabase.rpc('record_candidate_reference_form_submission', {
  ...
  if (error) {
    console.warn('[reference submission] rejected:', error.message);
